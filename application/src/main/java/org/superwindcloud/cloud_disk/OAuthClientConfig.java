  ...
  private ClientRegistration buildGoogleRegistration(Environment env) {
    String clientId =
        firstNonNull(
            ...
            env.getProperty("SPRING_SECURITY_OAUTH2_CLIENT_REGISTRATION_GOOGLE_CLIENT_ID"));
    String clientSecret =
        firstNonNull(
            env.getProperty("spring.security.oauth2.client.registration.google.client-secret"),
            env.getProperty("SPRING_SECURITY_OAUTH2_CLIENT_REGISTRATION_GOOGLE_CLIENT_SECRET"));
    ...
    String redirectUri =
        firstNonNull(
            ...
            env.getProperty("SPRING_SECURITY_OAUTH2_CLIENT_REGISTRATION_GOOGLE_REDIRECT_URI"),
            "{baseUrl}/login/oauth2/code/google");
    String issuer =
        firstNonNull(
            env.getProperty("spring.security.oauth2.client.provider.google.issuer-uri"),
            env.getProperty("SPRING_SECURITY_OAUTH2_CLIENT_PROVIDER_GOOGLE_ISSUER_URI"),
            "https://accounts.google.com");
    String scopeRaw =
        firstNonNull(
            env.getProperty("spring.security.oauth2.client.registration.google.scope"),
            env.getProperty("SPRING_SECURITY_OAUTH2_CLIENT_REGISTRATION_GOOGLE_SCOPE"),
            "openid,profile,email");
    List<String> scopes =
        Arrays.stream(scopeRaw.split(",")).map(String::trim).filter(s -> !s.isEmpty()).toList();
